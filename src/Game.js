...
function GameState(game) {
    ...
    function SpeedBoost(x, y, direction) {
        ...
        console.log(keyToAnims[key]);
        this.sprite.animations.add("move", keyToAnims[key], 10, true);
        this.sprite.play("move");
    }

    function Water(x, y, spriteKey, above) {
        var frameRate = 5;
        this.type = '';
        this.sprite = groups.waters.create(x * 50, y * 50, spriteKey);
        game.physics.arcade.enable(this.sprite);
        this.sprite.body.immovable = true;
        this.sprite.body.moves = false;
        switch (above) {
        ...
        this.playerTouch = function (playerSprite, waterSprite) {
            ...
            if (playerSprite.bottom > waterSprite.y + (playerSprite.height * COVERED)) {
                if (!snail.water.inWater) {
                    snail.water.inWater = true;
                    snail.water.entrySpeed = Math.abs(snail.sprite.body.velocity.y);
                    game.sound.play("water", snail.water.entrySpeed / 300);
                }
                snail.sprite.body.velocity.y -= ACCELERATION;
                snail.sprite.body.velocity.y += DRAG * snail.sprite.body.velocity.y;
            }
            if (cursors.up.isDown && playerSprite.body.velocity.y < JUMP_CUTOFF) {
                snail.jumping = true;
            }
            snail.water.wasIn = true;
        };
        ...
        for (i = 0; i < levelLayout.length; i += 1) {
            for (var j = 0; j < levelLayout[0].length; j += 1) {
                ...
                switch (levelLayout[i][j].toLowerCase()) {
                    ...
                    case 'w':
                        spriteKey = 'water';
                        currentTile = new Water(j, i, spriteKey, levelLayout[i - 1][j]);
